...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('reset-cash')
    .setDescription('รีเซ็ตเงินสดของผู้ใช้ (Admin เท่านั้น)')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addUserOption(opt => opt.setName('target').setDescription('เลือกผู้ใช้ที่ต้องการรีเซ็ต'))
    .addBooleanOption(opt => opt.setName('all').setDescription('รีเซ็ตเงินสดของผู้ใช้ทุกคน (ต้องเลือกเป็น True)')),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      await interaction.reply({ content: 'เกิดข้อผิดพลาดในการรีเซ็ตเงินสด', ephemeral: true });
    }
  }
};
